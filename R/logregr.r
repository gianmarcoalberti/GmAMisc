...
logregr <- function(data,oneplot=FALSE){
  ...
  p3 <- ggplot(df.diagn, aes(x=pred.prob, y=y, color=DepVar)) + geom_jitter(position=position_jitter(h=0.1), alpha=0.70) + xlim(0,1) + theme_bw() + labs(x = paste0("Predicted probability\n(AUC: ", auc, ")"), y="Dependent variable")
